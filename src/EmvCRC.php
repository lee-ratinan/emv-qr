...
class EmvCRC {
    ...
    /**
     * EmvCRC constructor.
     * @param string|null $crc
     * @param string|null $qr_string
     */
    public function __construct($crc = null, $qr_string = null)
    {
        if ( ! empty($crc))
        {
            $this->id = self::ID;
            ...
            {
                $qr_string = substr($qr_string, 0, -4);
                $digest = $this->CRC16HexDigest($qr_string);
                if ($crc == $digest)
                {
                    $this->value = $crc;
                    $this->description = 'CRC VALUE: ' . $crc;
                    $this->error = FALSE;
                } else
                {
                    $this->error = TRUE;
    ...
    /**
     * Generate the string for CRC
     * @param string $string The whole string to digest
     * @return string
     */
    public function generate($string)
    {
        $string .= self::ID . self::LENGTH;
        return self::ID . self::LENGTH . $this->CRC16HexDigest($string);
    }
